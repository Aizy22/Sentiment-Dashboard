...
const Dashboard = ({ navigateTo }) => {
  ...
  const API_BASE_URL = "https://a34b-102-212-60-154.ngrok-free.app";
  ...
  const fetchYouTubeCommentsAnalysis = async (videoUrl) => {
    ...
    setError(null);
    try {
      // CORRECTED: Method changed to POST, URL is now clean, body contains video_url
      const response = await fetch(`${API_BASE_URL}/youtube/youtube/comments`, {
        method: 'POST', // <-- THIS IS NOW POST
        headers: {
      ...
      // This line is correct and will execute upon successful API response
  ...
  const analyzeSentiment = async (textToAnalyze) => {
    ...
    try {
      ...
      const data = await response.json();
      setOverallSentimentData(data); // Store the overall sentiment result
